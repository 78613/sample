...
overflow:
    printk(KERN_ERR "\n\n>>>>>>>>>>>>>> CRITICAL OVERFLOW ERROR IN var_calc!!!!!!\ n\n");
    return -EINVAL;
}


static int
__init hello_start( void )
{
    ...
    if (!min_values) { 132      printk(KERN_ERR "unable to allocate memory for min_values\n");
        return 0;
    }
